...
func Wrap(err error, message string) error {
	...
	if _, hasStack := err.(*withStack); hasStack {
		return msgErr
	}
	return &withStack{
		msgErr,
...
func stackFromError(err error) *stack {
	for err != nil {
		if serr, ok := err.(*fundamental); ok {
			return serr.stack
		}
		if serr, ok := err.(*withStack); ok {
			return serr.stack
		}
		cause, ok := err.(causer)
		if !ok {
...
func CodeWrap(codeErr *CodeError, err error, msg string) error {
	...
	return &withStack{
		error: &withMessage{cause: codeErr, msg: fmt.Sprintf("%s: %s", msg, err)},
		stack: stack,
	}
}
